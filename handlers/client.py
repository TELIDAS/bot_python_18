...
async def hello(message: types.Message):
    await bot.send_message(
        message.chat.id, "Hello im your first bot", reply_markup=client_kb.start_markup
    )
...
async def quiz_1(message: types.Message):
    ...
    button_call_1 = InlineKeyboardButton(
        "Следующая Викторина", callback_data="button_call_1"
    )
    markup.add(button_call_1)
    question = "Who invented Python"
    answers = ["Voldemort", "Harry Potter", "Linus Torvalds", "Guido Van Rossum"]
    await bot.send_poll(
