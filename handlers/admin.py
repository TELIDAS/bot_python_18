...
async def is_admin_function(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        await bot.send_message(
            message.chat.id, "Hello, u r not allowed to this function"
        )
...
def register_handler_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(
        load_photo, content_types=["photo"], state=ShowsAdminStates.photo
    )
    dp.register_message_handler(
        load_title, content_types=["text"], state=ShowsAdminStates.title
    )
    dp.register_message_handler(
        load_description, content_types=["text"], state=ShowsAdminStates.description
    )
